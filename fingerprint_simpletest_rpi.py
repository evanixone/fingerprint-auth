...
def save_fingerprint_image(folder, base_filename):
    """Scan fingerprint then save image to filename."""
    # Determine the filename
    filename = os.path.join(folder, base_filename)
    count = 0
    while os.path.exists(filename + str(count) + ".png"):
        count += 1
...
while True:
    ...
    if c == "s":
        if save_fingerprint_image("fingerprint-server/fingerprints-repeat", "fingerprint"):
